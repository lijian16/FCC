...
class EffectiveClassMarginLoss(nn.Module):
    def __init__(self, 
        ...
        if 'cifar' in self.dataset:
            n = torch.tensor(torch.load('cifar_image_count.pkl')).float()
            #print(n.shape)
            #exit(0)
    ...
    def compute_weight(self, cls_score):
        ...
        pos_w = (n_neg.pow(1/4)/(n_pos.pow(1/4) + n_neg.pow(1/4))).pow(-1).log().to('cuda')
        neg_w = (n_pos.pow(1/4)/(n_pos.pow(1/4) + n_neg.pow(1/4))).pow(-1).log().to('cuda')
